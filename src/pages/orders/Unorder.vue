<template>
  <a-card>
    ...
    >
      <a-form :form="deliverForm">
        <a-form-item label="请填写快递单号：">
          <a-input
            allow-clear
            autocomplete="off"
            v-decorator="[
...
export default {
  ...
  methods: {
    ...
    handleDelivereOk() {
      const { selectedKey, orderShowList, orderList } = this;
      const _this = this;
      console.log(selectedKey);
      this.deliverForm.validateFields((err, values) => {
        if (!err && values) {
          orderList[selectedKey - 1].deliver_no = values.deliver_no;
          orderShowList[selectedKey - 1].deliver_no = values.deliver_no;
          orderList.splice(selectedKey - 1, 1);
          this.confirmLoading = true;
          setTimeout(() => {
    ...
    export2Excel() {
      ...
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, data, "未发货订单列表");
      });

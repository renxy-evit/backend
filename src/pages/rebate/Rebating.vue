  ...
  <a-card>
    ...
    <div>
      <standard-table :columns="columns" :dataSource="rebateShowList">
        ...
        <template slot="rebate_action" slot-scope="{ record }">
          <a-button
            @click="handleAllIssue(record.key)"
            type="primary"
            size="small"
            >全部发放</a-button
          >
          <a-button
            @click="handleSingleIssue(record.key)"
...
const columns = [
  ...
  {
    title: "操作",
    dataIndex: "rebate_action",
    align: "center",
    scopedSlots: { customRender: "rebate_action" },
  },
];
const rebateList = [
  {
...
export default {
  ...
  methods: {
    ...
    handleAllIssue(key) {
      const { rebateList, rebateShowList } = this;
      const _this = this;
      this.$confirm({
        title: "全部发放",
        content: (
          <div style="color:red;">
            此操作将发放申请人名下所有奖励金，请确认所有返佣订单有效！
          </div>
        ),
        onOk() {
          // rebateList[selectKey - 1 ].type="2"
          rebateList.splice(key - 1, 1);
          rebateShowList.splice(key - 1, 1);
          _this.$message.success("全部发放成功!");
          console.log("OK");
        },
        onCancel() {
          console.log("Cancel");
    ...
    handleSingleIssue(key) {
      const { rebateList, rebateShowList } = this;
      const _this = this;
      this.$confirm({
        title: "单笔发放",
        content: <div style="color:red;">您是否确认发放此笔订单奖励？</div>,
        onOk() {
          // rebateList[selectKey - 1 ].type="2"
          rebateList.splice(key - 1, 1);
          rebateShowList.splice(key - 1, 1);
          _this.$message.success("单笔发放成功!");
        },
        onCancel() {
          console.log("Cancel");
        },
